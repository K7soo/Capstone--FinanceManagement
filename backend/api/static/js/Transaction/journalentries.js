document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveJournalEntry() {
        const journalEntryData = {
            TransactionType_FK: parseInt(document.getElementById('transactionType').value),
            TRTemplate_FK: parseInt(document.getElementById('addTemplate').value),
            Entry_No: document.getElementById('entryCode').value,
            EntryStatus_FK: 1, // Default to pending (can be dynamic if needed)
            Entry_Date: document.getElementById('entryDate').value,
            EntryParticulars: document.getElementById('entryDescription').value,
            Created_By: 1 // Adjust based on the current user logic
        };

        const journalDetailsData = [];
        document.querySelectorAll('#accounting-entries table tbody tr').forEach(row => {
            const accountFK = parseInt(row.querySelector('.account-code').value);
            const debitAmount = parseFloat(row.querySelector('.debit-input').value) || 0.0;
            const creditAmount = parseFloat(row.querySelector('.credit-input').value) || 0.0;
            journalDetailsData.push({
        ...
        const payload = {
            ...
            journal_details: journalDetailsData
        };
    ...
    document.getElementById('addEntryBtn')?.addEventListener('click', (event) => {
        event.preventDefault(); // Prevent default form submission
        saveJournalEntry(); // Call the function to save the journal entry
    });

    // Update Journal Entry (PUT)
    function updateJournalEntry(entryId) {
        const journalEntryData = {
            TransactionType_FK: parseInt(document.getElementById('transactionType').value),
            TRTemplate_FK: parseInt(document.getElementById('addTemplate').value),
            Entry_No: document.getElementById('entryCode').value,
            EntryStatus_FK: 1, // Adjust as per your logic
            Entry_Date: document.getElementById('entryDate').value,
            EntryParticulars: document.getElementById('entryDescription').value,
            Created_By: 1 // Adjust based on current user logic
        };
